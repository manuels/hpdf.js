...
class Destination
  ...
  setXYZ: (left, top, zoom) ->
    ccall(@dst, 'HPDF_Destination_SetXYZ', 'number', ['number', 'number', 'number', 'number'], [@dst, left, top, zoom])
...
class Page
  ...
  createDestination: ->
    ret = ccall(this, 'HPDF_Page_CreateDestination', 'number', ['number'], [@page])
    new Destination(@hpdf, ret)
...
class HPDF
  ...
  setOpenAction: (dst) ->
    ccall(this, 'HPDF_SetOpenActoin', 'number', ['number', 'number'], [@hpdf, dst.dst])
